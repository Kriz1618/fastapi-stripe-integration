...
async def create_stripe_customer(request: CreateCustomerRequest, db: Session = Depends(get_db)):
    ...
    if user.stripe_customer_id:
        return {"message": "Customer already exists", "customer_id": user.stripe_customer_id}
...
async def create_checkout_session(request: CreateCheckoutRequest, db: Session = Depends(get_db)):
    ...
    if not user.stripe_customer_id:
        raise HTTPException(status_code=400, detail="User doesn't have Stripe customer ID")
    ...
    price_id = plan_mapping.get(request.plan)
    if not price_id:
...
async def handle_checkout_completed(session, db: Session):
    ...
    try:
        ...
        plan = get_plan_from_price_id(price_id) if price_id else "unknown"
        ...
        if not current_period_start or not current_period_end:
            logger.error(f"Missing period data in subscription: start={current_period_start}, end={current_period_end}")
            return
...
async def handle_payment_succeeded(invoice, db: Session):
    ...
    try:
        logger.info(f"Processing payment succeeded for invoice: {invoice.get('id', 'unknown')}")

        # Use service method to extract subscription_id
        subscription_id = stripe_service.extract_subscription_id_from_invoice(invoice)

        if not subscription_id:
            logger.error(f"No subscription found in invoice: {invoice.get('id', 'unknown')}")
            return
...
async def handle_invoice_upcoming(invoice, db: Session):
    ...
    try:
        ...
        if not subscription_id:
            logger.error(f"No subscription found in upcoming invoice: {invoice.get('id', 'unknown')}")
            return
        ...
        invoice_amount = invoice.get('amount_due', 0) / 100  # Convert from cents
        ...
        logger.info("SUBSCRIPTION_RENEWAL_REMINDER", extra={
            "user_id": user.id,
